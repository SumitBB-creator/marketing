...
class LeadService {
    ...
    async getLeads(params) {
        ...
        if (params.requestingUserRole === 'marketer' && params.requestingUserId) {
            const assignments = await database_1.default.marketerAssignment.findMany({
    ...
    async claimLead(leadId, marketerId) {
        return database_1.default.$transaction(async (tx) => {
    ...
    async deleteLead(leadId, userId, userRole) {
        ...
        if (userRole === 'admin' || userRole === 'super_admin') {
            return await database_1.default.lead.delete({
                where: { id: leadId }
            });
        }
    ...
    async optOutLead(leadId, userId) {
        return database_1.default.$transaction(async (tx) => {
            const lead = await tx.lead.findUnique({ where: { id: leadId } });
            if (!lead)
                throw new Error('Lead not found');
